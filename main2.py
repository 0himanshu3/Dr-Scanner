...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.setGeometry(150, 80, 1150, 780)

        # Data Storage
        self.image_sources = []
        self.original_images = []
        self.processed_images = []
        self.ocr_data_results = []
        self.ocr_texts_preview = []
        ...
        self.save_structured_text_pdf_button = None
    ...
    def initUI(self):
        ...
        sidebar_frame = QFrame(); sidebar_frame.setLayout(sidebar_layout); sidebar_frame.setFixedWidth(270);
        ...
        self.scan_button = QPushButton("⚙️ Process Documents")
        self.save_searchable_pdf_button = QPushButton("💾 Save Searchable PDF")
        self.save_structured_text_pdf_button = QPushButton("📄 Save Structured Text PDF")
        self.search_button = QPushButton("↗️ Open Save Folder")
        ...
        sidebar_layout.addWidget(self.save_structured_text_pdf_button)
    ...
    def load_files(self):
        ...
        for i, path in enumerate(files):
            ...
            if ext == ".pdf": pages = load_pdf_as_images(path)
    ...
    def scan_finished(self):
        ...
        if self.auto_save_filename_searchable or self.auto_save_filename_structured_text:
            ...
            if self.auto_save_filename_structured_text:
                 try:
                     ...
                     original_dims = [(img.shape[1], img.shape[0]) for img in self.original_images]
        ...
        first_valid_index = -1;
        if self.processed_images: first_valid_index = next((i for i, img in enumerate(self.processed_images) if img is not None), -1);
        self.current_preview_index = first_valid_index if first_valid_index !=-1 else (0 if self.original_images else -1)
        if self.current_preview_index != -1: self.update_preview(use_original=False)
    ...
    def update_preview(self, use_original=False):
        if not (0 <= self.current_preview_index < len(self.original_images)):
    ...
    def save_structured_text_pdf_manual(self):
        ...
        try:
            ...
            if len(self.original_images) != len(ocr_data_to_save):
                 ...
                 original_dims = [(img.shape[1], img.shape[0]) for img in self.original_images[:min_len]]
    ...
    def open_saved_documents_folder(self):
        ...
        try: success = QDesktopServices.openUrl(QUrl.fromLocalFile(target_folder));
    ...
    def update_button_states(self, scanning=False, loading=False):
        ...
        if self.save_structured_text_pdf_button: self.save_structured_text_pdf_button.setEnabled(has_any_valid_ocr_data and not is_busy)

        if self.search_button: self.search_button.setEnabled(True)
