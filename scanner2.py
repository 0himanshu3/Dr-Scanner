...
def detect_document(image):
    ...
    # Converting to gaussian blur and reducing the noise
...
def preprocess_image(image):
    ...
    # Defining a small kernel for morphological operations
    kernel = np.ones((1, 1), np.uint8)

    # Applying morphological opening to remove small noise
    opened = cv2.morphologyEx(binary, cv2.MORPH_OPEN, kernel)
    # Applying morphological closing to fill small holes
